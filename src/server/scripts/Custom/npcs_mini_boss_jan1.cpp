...
SDAuthor: DopeAttack
SDComment: 15% Wird überarbeitet
...
#define SAY_SAPHOS_BERSERK			"Jetzt bin ich W�tend!"
#define SAY_SAPHOS_FREEZE			"Sp�rt die Macht der Frostzauber!"
...
#define SAY_HAKKAR_BERSERK			"Sp�rt meinen Zorn"
...
enum TaudanSpells
{
	...
	S�ULE_DES_LEIDS			=						59727,
	STRUM_DER_TRAUER		=						59772 // zuf�lliges ziel
};

- S�ule des Leids (59727)			
- Schattenschleier (24674)			(auf ziel mit aggro)
- Sturm der Trauer (59772)			(auf zuf�lliges ziel)

// Tau'dan Schreie

#define SAY_TAUDAN_AGGRO		"F�hlt die Aura der Dunkelheit"
#define SAY_TAUDAN_DEATH		"Nein! ... wieso??"
#define SAY_TAUDAN_SCHLAG		"Sp�rt die Macht des Flammenschlags"
...
enum AuraleusSpells
{
	...
	STATISCHE_�BERLADUNG	=						59795,
	KETTENBLITZSCHLAG		=						63479,
	DONNER					=						53630,
	MANA_ENTZ�NDEN			=						46543, //selten alle 4 min zuf�lliges ziel
	�BERLADEN				=						63481, // bei 75% ,50%,25% 
		...
		struct npc_hakkarAI : public ScriptedAI
		{
			...
			void UpdateAI(const uint32 uiDiff)
			{
				...
				if(Phase2Timer <= uiDiff)
				{
					...
					me->RemoveAllAuras();
					me->DeleteThreatList();
					// In Phase 2 4 Teufelsj�ger
						 if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM,0))
							for (uint8 i = 1; i <= 4; i++)
							{
								me->SummonCreature(TEUFELSJ�GER_BESCHW�REN, pTarget->GetPositionX(), pTarget->GetPositionY(), pTarget->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 60000);
							}
