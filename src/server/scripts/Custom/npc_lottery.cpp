    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        switch(uiAction)
        {
            case GOSSIP_ACTION_INFO_DEF:
                ...
                char msg[500];
                sprintf(msg, "Viel Glück, $N. ihr Ticket und die Zahl %i", id+1);
    ...
    struct npc_lottoAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            if (IsEventActive(EVENT_BLOODYLOTTO))
            {
                if (me->IsVisible())
                {
                    ...
                    do
                    {
                        ...
                        MailDraft("Lotterie Preis!", "Herzlichen Glückwunsch sie haben Extended-Lotto gewonnen!")
                            .AddMoney(reward)
            ...
            else
            {
                ...
                if (SayTimer <= diff)
                {
                    me->MonsterSay("Kommt her und kauft Lotterie Tickets! Mit nur 50 Gold zum Millionär werden!", 0, NULL);
                    SayTimer = 1800*IN_MILLISECONDS;
                }
                else SayTimer -= diff;
            }
